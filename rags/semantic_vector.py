...
from llama_index.schema.TextNode import TextNode
...
def taxpipe(username,nodes):
    heir_node_parser = HierarchicalNodeParser.from_defaults(
    chunk_sizes=[2048, 512, 128]
)
    for d in nodes:
        print(d.metadata['entities'])
        node_parser = HierarchicalNodeParser.from_defaults()
        nodes = node_parser.get_nodes_from_documents(d.text)        
        ...
        pipe = pipeline("text-classification", model="TimSchopf/nlp_taxonomy_classifier")
        tokenizer = AutoTokenizer.from_pretrained('TimSchopf/nlp_taxonomy_classifier')
        node = TextNode(text=d.text, metadata=d.metadata + pipe(d['document_title'] + tokenizer.sep_token + (d.get('section_summary') or ''),return_all_scores=True))
        taxonomy_vector_store.add(node)
...
def getJobs():
    response = semantic_query_engine.query(f"Use the query engine tool to get me a list of jobs from 5 linkedin queries based on the highest level skills that {username} is capable of \n return format should be as a python tuple, Example: ('queries',['full stack developer', 'Backend Engineer', 'UX designer'])")
    return response
def parserJobs(keywords):
    ...
    agent.chat("Instructions: based on the job descriptions scraped from linkedin, create a persona profile and example documents that someone qualified for the job would have. Criteria: 1. make up a person with a random name and describe their strengths, experiences, and projects that qualify them for any of the linkedin jobs. 2. generate collection of files that demonstrate your person has the skill to qualify for the job. 3. generate 1 resume, 1 CV for a linkedin job, 3 coding projects.")
